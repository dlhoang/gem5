...
output-directory including these files:
    config.ini, config.json --- the full configuration of
                                all the internal components of the simulation.

    program.out, program.err --- the output of the executable program

    stats.txt --- statistics from the simulation, such as the number of
                  instructions executed, the cache hit ratios, etc., etc.
...
If you'd like to add command-line parameters to this script, you can do so by
editing the get_options() command to add another call to parser.add_option().
The parsed command-line parameters will be available create_cpu() via the
options variable.
...
"""
Setup a simulation to run the specified program,
writing output to the specified output directory.

Optionally supports running with a different
(faster) CPU for the first instrunctions.
"""
def run_system_with_cpu(
    ...
    if warmup_cpu_class:
        ...
        exit_event = m5.simulate(max_tick)
    eprint("Done simulation @ tick = %s: %s" % (m5.curTick(),
        exit_event.getCause()))
    m5.stats.dump()
...
def get_options():
    ...
    if not options.directory:
        eprint("""You must set --directory to the name of an
    ...
    # Some features are not supported by this script, but are added to
    # parser by Options.addSEOptions and Options.addCommonOptions

    # I check for these here to avoid confusion
    # If you are failing an assertion here, removing the assertion
    # will not make the option work.
