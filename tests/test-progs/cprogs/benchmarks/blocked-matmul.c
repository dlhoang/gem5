 ...
 */
void blocked_matmul(myfloat * restrict A, myfloat * restrict B,
    myfloat * restrict C, int I, int K, int J) {
    assert(I % 2 == 0);
    assert(J % 2 == 0);
    assert(K % 2 == 0);
    for (int i = 0; i < I; i += 2) {
...
uint64_t read_tsc(void) {
...
#endif
    /*
     * Embed the assembly instruction 'rdtsc', which should not
     * be relocated (`volatile').
     *  The instruction will modify r_a_x and r_d_x, which the
     *  compiler should map to
     ...
     *
     * Note that if you do not (correctly) specify the side effects of an
     * assembly operation, the
     * compiler may assume that other registers and memory are not affected.
     * This can easily lead to cases where your program will produce
     * difficult-to-debug wrong answers when optimizations are enabled.
     */
